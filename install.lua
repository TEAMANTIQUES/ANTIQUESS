...
local RunBot = io.open("ANTIQUESS", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/ANTIQUESS
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr ANTIQUESS.lua
wget "https://raw.githubusercontent.com/TEAMANTIQUES/ANTIQUESS/main/ANTIQUESS.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./ANTIQUESS.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S ANTIQUESS -X kill
screen -S ANTIQUESS ./ANTIQUESS
done
]])
RunTs:close()
end
